...
## Modelo Relacional · Versión 3.6

**Documento de revisión arquitectónica — análisis campo por campo · Módulo Finanzas** *Mayo 2026*

---

## Cambios respecto a v3.5

| \# | Tabla | Cambio | Detalle |
| :---: | :---- | :---- | :---- |
| 1 | `socios` | **Renombrada** | `socios` → `distribucion_ganancias` — el nombre describe el contenido, no las personas |
| 2 | `distribucion_ganancias` | **Fila Gastos agregada** | El 30% de gastos pasa a ser una fila explícita — los 3 conceptos suman siempre 100% |
| 3 | `distribucion_ganancias` | **Campos nuevos** | Se agregan `activo` BOOLEAN y `archivado` BOOLEAN — control de estado independiente |
| 4 | `distribucion_ganancias` | **Regla del 100%** | Σ(porcentaje WHERE activo=true) ≤ 1.0000 — controlado por la aplicación |
| 5 | `envios` | **Postergada** | Revisión diferida a etapa posterior — no bloquea el cierre del módulo Finanzas |
| 6 | Tabla de auditoría | **`distribucion_ganancias` actualizada** | Reemplaza la fila `socios` en la tabla de criterios de auditoría |
| 7 | `clientes` | **Campo eliminado** | `grupos` removido — sin uso operativo definido por el momento |
| 8 | Documentación | **Corrección gobernanza proveedores** | `insumo_proveedor.precio_referencia` es fuente de entrada, no disparo automático — la aplicación ejecuta la actualización explícitamente (consistente con V8) |
...
| `distribucion_ganancias` | — | ✅ | Interesa saber si cambió un porcentaje |
...
| Tabla | Prioridad | Descripción |
| :---- | :---: | :---- |
| `distribucion_ganancias` | P1 | Parametriza cómo se reparten las ganancias. 3 conceptos explícitos (Meme 40%, Pety 30%, Gastos 30%). Soporta activar/archivar conceptos. |
...
| `envios` | P3 | Datos de despacho. **Postergada — revisión diferida a etapa posterior.** |

**Relaciones internas del módulo:**

distribucion\_ganancias ◄── transacciones (distribución automática 40/30/30 al facturar)
...
## Detalle de tablas modificadas en v3.6

### `distribucion_ganancias` *(antes `socios` — rediseñada en v3.6)*

| Columna | Tipo | Descripción |
| :---- | :---- | :---- |
| `id` | SERIAL PK | Identificador interno |
| `concepto` | VARCHAR(60) | Nombre del receptor o categoría (ej: 'Meme', 'Pety', 'Gastos') |
| `porcentaje` | NUMERIC(5,4) | Proporción del total (ej: 0.4000 \= 40%) |
| `activo` | BOOLEAN | TRUE \= participa en el cálculo de distribución actual |
| `archivado` | BOOLEAN | TRUE \= retirado definitivamente, no vuelve a activarse |
...
2\. Σ(porcentaje WHERE activo \= true) ≤ 1.0000
   ...
   Controlado en capa de aplicación — consistente con V8 (sin triggers complejos).

3\. Una fila con activo \= false pero archivado \= false

   puede reactivarse en el futuro.

---

## Detalle de tablas modificadas en v3.4 y v3.5
...
| `precio_referencia` | NUMERIC(10,2) | Precio de referencia del proveedor para este insumo. Actúa como fuente de entrada — la aplicación lo usa para actualizar `insumos.costo_paquete` de forma explícita, no automática (consistente con V8) |
...
║  distribucion\_ganancias ◄── transacciones            ║
...
| V13 | Sin validación de porcentajes por trigger | La regla Σ porcentajes ≤ 100% en `distribucion_ganancias` la controla la aplicación — consistente con V8 |

---

## Resumen cuantitativo

| Módulo | Tablas v3.5 | Tablas v3.6 | Cambio |
| :---- | :---: | :---: | :---- |
| Productos | 7 | 7 | Sin cambios |
| Comercial | 4 | 4 | Sin cambios |
| Finanzas | 4 | 4 | Sin cambios en cantidad — `socios` renombrada y rediseñada como `distribucion_ganancias` |
| **Total** | **15** | **15** |  |

---

*Arquitectura Final · MemyDeni · v3.6 · Mayo 2026*  
