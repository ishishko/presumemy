...
1. **Crear carpetas** vacías: `app/`, `app/{shell,state,styles}`, `shared/{ui,lib,api,config}`, `modules/{insumos,productos,clientes,presupuestos,finanzas,categorias,ajustes,dashboard,auth}/` (cada módulo con `components/` cuando aplique).
...
3. **Singletons de orquestación a `app/state/`:** `composables/useEditorMode.ts` → `app/state/editorMode.ts`; `composables/useCreateTrigger.ts` → `app/state/createTrigger.ts`. (C15: `app` puede importar de todo; nadie de `modules`/`shared` importa de `app`.)
