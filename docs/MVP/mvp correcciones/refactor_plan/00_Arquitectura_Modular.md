...
└── modules/    # UN dominio = UNA carpeta autocontenida (barrel index.ts)
    ...
    │   ├── components/{StockBar,ProveedoresEditor,InsumosTable,CategoriaPills,CategoriaDeleteDialog}.vue
    ...
    ├── productos/    Page · ProductoDetalle · components/{ProductCard,BomEditor,CategoriaPills,CategoriaDeleteDialog} · store (incluye CRUD categorías) · api · pricing · types · schema · index
...
**Nota sobre categorías:** No hay módulo `categorias/`. Las categorías de insumo y de producto viven en sus respectivos módulos (`modules/insumos/` y `modules/productos/`). Los componentes de UI (`CategoriaPills`, `CategoriaDeleteDialog`) se duplican en cada módulo porque cada uno tiene su propio store/api de categorías. Si en el futuro se detecta que la lógica es idéntica y vale la pena extraer, se puede crear un módulo `categorias/` compartido (Regla de Tres).
...
| G4 (CategoriaPills/Dialog, PresupuestoDoc) | `modules/insumos/components/`, `modules/productos/components/`, `modules/presupuestos/` |
