# G5.1 — `modules/dashboard/DashboardPage.vue`
...
| | |
|---|---|
| **Ruta destino** | `web/src/modules/dashboard/DashboardPage.vue` |
...
1. **(DRY)** `money`/`moneyShort` → `formatMoney` (con variante compacta `moneyShort` quizá a `shared/lib/format`).
2. **(SRP)** Extraer el gráfico a `modules/dashboard/components/WeeklyChart.vue` (presentacional; recibe `data`). **Nota:** los datos están hardcodeados — anotar como deuda (debería venir del store/stats); no se inventa backend aquí, pero se aísla.
3. **(reuso)** KPIs → `BaseKpi`; bloques → `BaseCard`; badges de estado → `StatusBadge` con el **mapa de estados FSM** (compartir con `PresupuestosPage`, ver G5.5 — extraer `statusTones` a un módulo común `modules/presupuestos/estado.ts`).
...
Crea `modules/dashboard/components/WeeklyChart.vue`. Consume `BaseKpi`, `BaseCard`, `StatusBadge`, `StockBar`, `formatMoney`, módulo común de estados (`modules/presupuestos/estado.ts`).
...
## Riesgos / notas
- **Deuda visible:** `chartData` y "Mayo 2026" están hardcodeados. Aislar en `modules/dashboard/components/WeeklyChart.vue` y dejar TODO para alimentarlo del backend (fuera de alcance del refactor de estilos).
- El módulo común de estados (tone+label+transiciones FSM) se comparte con G5.5; definirlo una vez en `modules/presupuestos/estado.ts`.
