...
| **LOC actuales** | 0 (no existe; tampoco la carpeta `utils/`) |
| **Tipo** | crear |
| **Dependencias** | ninguna |
| **Consumidores** | `InsumosView`, `FinanzasView`, `PresupuestoEditor`, `PresupuestoDoc`, `ProductoDetalle`, y toda vista con montos |
...
1. **(SRP)** Crear `utils/format.ts` con funciones puras exportadas:
...
```
En las vistas: `import { formatMoney } from '@/utils/format'` y reemplazar `money(x)` → `formatMoney(x)`.
...
- `vue-tsc` ok.
- **Test unitario** (`utils/__tests__/format.spec.ts`): casos de enteros, decimales, cero, y sufijo MXN. (Beneficio de testabilidad SOLID.)
...
- Confirmar alias `@/utils/*` en `tsconfig`/`vite` (ya se usa `@/` en todo el proyecto).
