...
| | |
|---|---|
| **Ruta** | `web/src/utils/stock.ts` |
| **Grupo / orden** | G1 (datos/utils) · 2º |
| **LOC actuales** | 0 (no existe) |
| **Tipo** | crear |
| **Dependencias** | ninguna |
| **Consumidores** | `InsumosView` (G5.4), `InsumoDetalle` (G6.5), `StockBar` (G3.6), `StatusBadge` (G3.2) |
...
1. **(SRP/C8)** Crear `utils/stock.ts` con el **modelo canónico de 4 niveles** (el de `InsumoDetalle`):
   ```ts
...
3. **(compat lista)** Para la tabla de `InsumosView` (que hoy maneja 3 chips), exponer `nivelColapsado(nivel): 'critico'|'bajo'|'ok'` que mapea `sin_unidades`→`critico`. Así la vista no pierde su UI de 3 estados pero comparte la regla.
...
- **Después:** `utils/stock.ts` único; `StockBar`/`StatusBadge`/vistas lo consumen.

## Criterios de aceptación
- `vue-tsc` ok.
- **Test unitario** (`utils/__tests__/stock.spec.ts`): `getNivel(0,10)='sin_unidades'`, `(2,10)='critico'`, `(8,10)='bajo'`, `(12,10)='ok'`; bordes `stock==min*0.2`, `minimo==0`.
- `nivelColapsado` cubierto.

## Riesgos / notas
- **Cambio de comportamiento a confirmar (C8):** `InsumosView` hoy usa umbral `0.5`; el canónico adopta `0.2`. Validar en el piloto (G5.4) que la lista de insumos "bajos/críticos" sigue teniendo sentido para producto. Si se rechaza, parametrizar el umbral en `getNivel(stock, minimo, { criticoFactor })`.
