...
Crea `utils/presupuestoEstado.ts`, `EstadoDropdown.vue`. Consume `DataTable`, `RowActions`, `FilterChips`, `StatusBadge`, `formatMoney`/`formatDate`.

## Criterios de aceptación
- `vue-tsc` ok; FSM de estados (transiciones, confirmación al facturar/cancelar, update optimista + revert) idéntica.
- `statusTones`/`TRANSITIONS` definidos **una sola vez** y reusados por Dashboard.
...
- `StatusBadge` necesita tonos para todos los estados FSM (`borrador/en_curso/cerrado/facturado/cancelado/enviado`); definir el mapa estado→tone en `presupuestoEstado.ts` y traducir a los tonos de `StatusBadge` (G3.2).
