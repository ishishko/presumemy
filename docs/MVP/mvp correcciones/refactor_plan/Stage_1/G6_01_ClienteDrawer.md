# G6.1 — `modules/clientes/ClienteDrawer.vue`
...
| | |
|---|---|
| **Ruta destino** | `web/src/modules/clientes/ClienteDrawer.vue` |
...
| **Consumidores** | `ClientesPage` (G5.2) |
...
2. **(SRP)** Extraer `modules/clientes/components/ContactosEditor.vue` (v-model de `contactos[]`; maneja add/remove/setPrincipal, límite 3, regla "siempre uno principal"). Reutiliza `FloatingSelect` para el canal y `FloatingField` para el valor.
...
Crea `modules/clientes/components/ContactosEditor.vue`. Consume `DrawerShell`, `FloatingField`, `FloatingSelect`, `BaseButton`, `ConfirmDialog`, store, `useDirty`.
